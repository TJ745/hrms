import { headers } from "next/headers";
import { redirect } from "next/navigation";
import { auth } from "@/lib/auth";
import { prisma } from "@/lib/prisma";
import { getResignations } from "@/actions/hr-operations.actions";
import { ResignationsClient } from "@/components/modules/resignations/resignations-client";

export default async function ResignationsPage({
  params,
}: {
  params: Promise<{ orgSlug: string }>;
}) {
  const { orgSlug } = await params;
  const session = await auth.api.getSession({ headers: await headers() });
  if (!session) redirect("/login");

  const user = await prisma.user.findUnique({
    where: { id: session.user.id },
    select: { organizationId: true, systemRole: true, employee: { select: { id: true } } },
  });
  if (!user?.organizationId) redirect("/select-org");

  const isHR = ["SUPER_ADMIN", "ORG_ADMIN", "HR_MANAGER"].includes(user.systemRole);

  const resignations = await getResignations({ organizationId: user.organizationId });

  return (
    <ResignationsClient
      resignations={resignations as any}
      isHR={isHR}
      currentEmployeeId={user.employee?.id ?? null}
      organizationId={user.organizationId}
      orgSlug={orgSlug}
    />
  );
}
